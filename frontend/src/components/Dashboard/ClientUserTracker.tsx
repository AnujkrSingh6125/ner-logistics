...
export default function ClientUserTracker({
  ...
  return (
    <div className="bg-slate-900/95 dark:bg-[#041217]/95 border border-teal-500/40 rounded-2xl p-4 shadow-xl text-slate-100 space-y-3.5 backdrop-blur-md">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div
            className={`p-2 rounded-xl border ${
              isAutoPolling
                ...
                : 'bg-teal-950/60 border-teal-800/60 text-teal-400'
            }`}
          >
          ...
          <div>
            <h3 className="font-bold text-xs sm:text-sm text-slate-100 flex items-center gap-1.5">
              ...
              {isAutoPolling && (
                <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[9px] font-mono font-bold bg-teal-950 text-emerald-300 border border-emerald-500/50 animate-pulse">
            ...
            <p className="text-[10px] text-slate-400">
              Query & track citizen driver GPS on demand with active live locking
            </p>
          </div>
        </div>
        ...
        {isAutoPolling && (
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold bg-rose-600 hover:bg-rose-500 text-white shadow-md shadow-rose-600/30 transition active:scale-[0.98]"
      ...
      >
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-9 pr-3.5 py-2.5 rounded-xl bg-slate-950 border border-teal-800/50 text-slate-100 placeholder-slate-500 text-xs focus:outline-none focus:border-teal-400 focus:ring-1 focus:ring-teal-400 transition"
          />
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          {!isAutoPolling ? (
            <button
              ...
              className="flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-bold bg-gradient-to-r from-teal-600 to-emerald-600 hover:from-teal-500 hover:to-emerald-500 text-white disabled:opacity-50 transition shadow-md shadow-teal-600/25 border border-teal-400/30"
            >
              {isFetching ? (
                <>
            ...
            <button
              type="button"
              onClick={handleStopFetching}
              className="flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-xl text-xs font-bold bg-rose-600 hover:bg-rose-500 text-white transition shadow-md shadow-rose-600/30"
            >
      ...
      <div className="flex items-center gap-1.5 text-[10px] text-slate-400 flex-wrap">
        <span className="font-mono text-teal-400 font-semibold">Quick Targets:</span>
        {['NER-CIT-84920', 'NER-CIT-49210', 'NER-CIT-GUEST'].map((sample) => (
          <button
            ...
            className="px-2 py-0.5 rounded-md bg-slate-800 hover:bg-teal-950 text-teal-300 font-mono text-[10px] border border-teal-800/50 hover:border-teal-400 transition"
          >
            {sample}
          </button>
        ))}
      ...
      {currentTelemetry && (
        <div className="p-3.5 rounded-xl bg-slate-950 border border-teal-500/40 shadow-inner space-y-2.5 animate-in fade-in duration-200">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            <div className="flex items-center gap-2">
              <div className="w-7 h-7 rounded-full bg-teal-950 border border-teal-400/50 flex items-center justify-center text-teal-400 font-bold text-xs">
                <User className="w-3.5 h-3.5" />
              </div>
              <div>
                <div className="font-bold text-xs text-slate-100 flex items-center gap-2">
                  <span>{currentTelemetry.full_name}</span>
                  <span className="font-mono text-[10px] text-teal-400 px-1.5 py-0.2 bg-teal-950/80 rounded border border-teal-800">
            ...
            <div className="flex items-center gap-1.5">
              ...
              >
                ...
                />
                {currentTelemetry.is_sharing_location ? 'LIVE GPS RADAR' : 'HISTORICAL POSITION'}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
            <div className="p-2 rounded-lg bg-slate-900 border border-teal-900/40">
            ...
            <div className="p-2 rounded-lg bg-slate-900 border border-teal-900/40">
              <span className="text-[10px] text-slate-400 block font-mono">Heading</span>
              <span className="font-mono font-bold text-teal-300 text-[11px] flex items-center gap-1">
          ...
          {(currentTelemetry.origin_hub || currentTelemetry.destination_hub) && (
            <div className="p-2 rounded-lg bg-slate-900/80 border border-teal-900/40 text-[11px] flex items-center justify-between text-slate-300">
              <span className="text-teal-400 font-mono text-[10px] font-bold">CORRIDOR:</span>
          ...
          <div className="flex items-center justify-between text-[10px] font-mono text-slate-400 pt-1 border-t border-slate-800/80">
            <span className="flex items-center gap-1">
              ...
              <span>Telemetry Ping #{pulseCount}</span>
